...
const onSubmit= async (e)=>{
  ...
  if(geolocationEnabled){
    const response = await fetch(`http:maps.googleapis.com/maps/api/geocode/json?
    address=${address}&key=AIzaSyDYEWeAYyfMgJxflFiINw22RGQTKPT95jQ`)
    const data = await response.json()
    geolocation.lat =data.result[0]?.geometry.location.lat ?? 0

    geolocation.lat =data.result[0]?.geometry.location.lng ?? 0
...
return <div className='profile'>
    <header>
       <p className="ageHeader">Create Listing</p> 
    </header>
    <main>
        <form onSubmit={onSubmit}>
         ...
         <div className='formRooms flex'>
            ...
            maxLength='50'
            minLength='1'
            required className="formInputSmall" />
            </div>
            </div>
